...
import com.util.SchemaUtil.Log
...
object Log2parquet {
  def main(args: Array[String]): Unit = {
    ...
    val sparkSession = SparkSession.builder().appName(this.getClass.getName)
      ...
      .getOrCreate()
    val lines = sparkSession.read.textFile(inputPath)
    import sparkSession.implicits._
    //设置过滤条件和切分条件，内部如果切割条件过多，那么，需要设置切割处理条件
    val ds: Dataset[Log] = lines.map(t => t.split(",", -1).toList).filter(_.length >= 85)
      .map(arr => {
        Log(
          arr(0),
